...
with st.form('Submisson_Form'):
    main_observatory = st.selectbox (label = 'Main Observatory', options = obs_options, index = obs_options.index(0))
    exoplanet_filter = st.selectbox (label = 'Exoplanet'       , options = obs_options, index =  pn_options.index(0))
